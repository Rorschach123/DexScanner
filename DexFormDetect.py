...
class DexDetect:
    ...
    def CheckingInsAndTypeValue(self,insNum,ins,dexHeader,insValue,strDes):
        ...
        #      print self.dexLoader.dexClassName[codeNum] + "--" + self.dexLoader.dexMethodName[codeNum]
            ...
            stringIdx = DexFormAnalyzer.endianToNormal(DexFormAnalyzer.stringListToIntList(self.dexLoader.mapfile[typeIdxOffset:typeIdxOffset+4],4),4) * 4 + dexHeader.stringIdxOff
            stringIdxImp = DexFormAnalyzer.endianToNormal(DexFormAnalyzer.stringListToIntList(self.dexLoader.mapfile[stringIdx:stringIdx+4],4),4)
    ...
    def CheckingInsAndClassValue(self,insNum,ins,dexHeader,insValue,methodName,className):
        ...
        if insNum % 2 == 0 and ins[insNum] == insValue:
            ...
            stringIdx = DexFormAnalyzer.endianToNormal(DexFormAnalyzer.stringListToIntList(self.dexLoader.mapfile[methodIdxOffset+4:methodIdxOffset+8],4),4) * 4 + dexHeader.stringIdxOff
            stringIdxImp = DexFormAnalyzer.endianToNormal(DexFormAnalyzer.stringListToIntList(self.dexLoader.mapfile[stringIdx:stringIdx+4],4),4)
            ...
            #type idx
            classStr = ""
            typeNum = DexFormAnalyzer.endianToNormal(DexFormAnalyzer.stringListToIntList(self.dexLoader.mapfile[methodIdxOffset+0:methodIdxOffset+2],2),2)
            ...
            stringIdxImp = DexFormAnalyzer.endianToNormal(DexFormAnalyzer.stringListToIntList(self.dexLoader.mapfile[classNameStrOffset:classNameStrOffset+4],4),4)
            stringlen = ord(self.dexLoader.mapfile[stringIdxImp])
            for strNum in range(0,stringlen):
                classStr += self.dexLoader.mapfile[stringIdxImp+strNum+1]
    ...
    def DetectApkApi(self,dexHeader):
        ...
        print "--Detc-- Start Checking if has Some Api"
        ...
        for codeNum in range(0,len(self.dexLoader.dexMethodCodes)):
            ...
            if resultDexClassLoader[0] > 0 and resultDexClassLoader[1] > 0:
                ...
                print "--Detc-- DexClassLoader : ClassName = " + self.dexLoader.dexClassName[codeNum] + " MethodName = " + self.dexLoader.dexMethodName[codeNum]
            else:
                if resultDexClassLoader[0] > 0:
                    countFindDexClassLoaderPossible += 1
                    print "--Detc-- Possible_DexClassLoader : ClassName = " + self.dexLoader.dexClassName[codeNum] + " MethodName = " + self.dexLoader.dexMethodName[codeNum]

        if countFindDexClassLoader != 0:
            print "--Detc-- Find DexClassLoader Api,Count number = %d" %(countFindDexClassLoader)
        else:
            print "--Detc-- Not Found DexClassLoader Api"
        if countFindDexClassLoaderPossible != 0:
            print "--Detc-- Find Possible DexClassLoader Api,Count number = %d" %(countFindDexClassLoaderPossible)
